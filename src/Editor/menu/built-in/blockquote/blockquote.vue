...
const isActive = computed<boolean>(() => {
  return (keyOfSelectionUpdate.value > 0 && editorRef.value?.commands.allBlockquote?.()) || false
})
//是否禁用
...
const onOperate = () => {
  if (isActive.value) {
    editorRef.value?.commands.unsetBlockquote?.()
  } else {
    editorRef.value?.commands.setBlockquote?.()
  }
}
</script>