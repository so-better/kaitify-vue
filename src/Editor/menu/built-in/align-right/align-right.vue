...
const isActive = computed<boolean>(() => {
  return (keyOfSelectionUpdate.value > 0 && editor.value?.commands.isAlign?.('right')) || false
})
//是否禁用
...
const onOperate = () => {
  if (isActive.value) {
    editor.value?.commands.unsetAlign?.('right')
  } else {
    editor.value?.commands.setAlign?.('right')
  }
}
</script>