...
const isActive = computed<boolean>(() => {
  return (keyOfSelectionUpdate.value > 0 && editorRef.value?.commands.isAlign?.('right')) || false
})
//是否禁用
...
const onOperate = () => {
  if (isActive.value) {
    editorRef.value?.commands.unsetAlign?.('right')
  } else {
    editorRef.value?.commands.setAlign?.('right')
  }
}
</script>