...
const isActive = computed<boolean>(() => {
  return (keyOfSelectionUpdate.value > 0 && editorRef.value?.commands.isAlign?.('justify')) || false
})
//是否禁用
...
const onOperate = () => {
  if (isActive.value) {
    editorRef.value?.commands.unsetAlign?.('justify')
  } else {
    editorRef.value?.commands.setAlign?.('justify')
  }
}
</script>