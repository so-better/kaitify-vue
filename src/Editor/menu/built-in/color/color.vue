...
const isActive = computed<(item: string) => boolean>(() => {
  return item => {
    return state.value.selection.focused() && (editor.value?.commands.isColor?.(item) ?? false)
  }
})
...
const setColor = (val: string) => {
  ...
  editor.value!.commands.setColor?.(val)
...
const unsetColor = () => {
  ...
  editor.value.commands.removeTextStyle?.(['color'])
  menuRef.value?.hidePopover()
}
</script>
