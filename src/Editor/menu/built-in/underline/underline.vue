...
const isActive = computed<boolean>(() => {
  return (keyOfSelectionUpdate.value > 0 && editorRef.value?.commands.isUnderline?.()) ?? false
})
//是否禁用
...
const onOperate = () => {
  if (isActive.value) {
    editorRef.value?.commands.unsetUnderline?.()
  } else {
    editorRef.value?.commands.setUnderline?.()
  }
}
</script>